...
function openModal(title,html,actions=[],trayHtml=''){
  ...
  if(topBack){topBack.innerHTML='<img src="ui-back.png?v=080-b21" alt="Back">';topBack.dataset.navigation='back';topBack.setAttribute('aria-label','Back');topBack.onclick=()=>backAction?.onClick?backAction.onClick(topBack):closeModal()}
...
function bindSetupTools(){
  $$('.setup-saved').forEach((sel,i)=>sel.onchange=async()=>{const d=listDecks().find(x=>x.id===sel.value);const p=$$('[data-player-setup]')[i];if(!p)return;if(!d){p.dataset.selectedDeckId='';p.dataset.selectedDeckName='';p.dataset.selectedDeckText='';return}p.dataset.selectedDeckId=d.id;p.dataset.selectedDeckName=d.name||'';p.dataset.selectedDeckText=d.deckList||'';p.querySelector('.setup-cmd1').value=d.commander1||'';p.querySelector('.setup-cmd2').value=d.commander2||'';p.querySelector('.setup-deck').value=d.deckList||'';await syncSetupSecondary(p)});
  $$('.setup-deck').forEach((input,i)=>input.addEventListener('input',()=>{const p=$$('[data-player-setup]')[i];if(!p?.dataset.selectedDeckId)return;const saved=p.dataset.selectedDeckText||'';if(input.value!==saved){p.dataset.selectedDeckId='';p.dataset.selectedDeckName='';p.dataset.selectedDeckText='';const sel=p.querySelector('.setup-saved');if(sel)sel.value=''}}));
...
function openingHandHtml(p){
  ...
  const deckLabel=p.deck?.sourceName||p.deck?.selectedDeckName||'Setup deck';
  return `<p>Review ${esc(p.displayName)}'s private opening hand. Only this player's cards should be visible while this screen is open.</p><p class="muted">Deck: ${esc(deckLabel)}</p>${mulliganNote}<div class="hand-panel"><h3>OPENING HAND (${p.deck.hand.length})</h3><div class="hand-strip">${p.deck.hand.map(c=>{const d=def(c.definitionId);return `<button class="hand-card" data-opening-card="${c.instanceId}">${d?`<img src="${imageOf(d)}" alt="${esc(d.name)}">`:'<span class="fallback">UNRESOLVED</span>'}</button>`}).join('')}</div></div>`
}
