...
function playerPanel(i){
  ...
  const commanderHint=fullTracked?'Selected Commander 1':'Commander name';
  return `<section class="player-setup player-setup-wide" data-player-setup="${i}"><div class="setup-player-row"><input class="setup-name" value="" placeholder="Player Name" aria-label="Player Name"></div>${hasDeck?`<div class="setup-deck-row"><input class="setup-deck-name" value="" placeholder="Select your deck" aria-label="Selected deck" readonly><select class="setup-saved" aria-label="Load saved deck"><option value="">LOAD SAVED DECK</option>${saved.map(d=>`<option value="${esc(d.id)}">${esc(d.name)}</option>`).join('')}</select><button type="button" class="setup-precon">PRECON</button></div><div class="setup-commander-row"><div class="setup-commander-primary"><input class="setup-cmd1" placeholder="${commanderHint}" ${fullTracked?'readonly':''}>${fullTracked?'<button type="button" class="setup-pick-cmd1" hidden></button>':''}</div><div class="setup-secondary-slot"><div class="setup-secondary-wrap" hidden><input class="setup-cmd2" placeholder="Selected Commander 2" ${fullTracked?'readonly':''}>${fullTracked?'<button type="button" class="setup-pick-cmd2" hidden></button>':''}</div></div><button type="button" class="setup-view-deck">VIEW DECK</button></div><textarea class="setup-deck" hidden></textarea>`:''}</section>`
}
...
async function openDeckCommanderPicker({deckText,primaryName='',secondary=false,onSelect}){
  ...
  openModal(secondary?'SELECT SECOND COMMANDER':'SELECT COMMANDER','<p>Resolving eligible cards from this deck only…</p>',[{label:'CANCEL',onClick:restoreParent}],restoreParent);
  try{
...
$('#deckPickCommander1').onclick=()=>openDeckCommanderPicker({deckText:$('#deckListInput').value,onSelect:d=>{$('#deckCommander1').value=d.name;$('#deckCommander2').value='';syncEditorSecondary()}});
$('#deckPickCommander2').onclick=()=>openDeckCommanderPicker({deckText:$('#deckListInput').value,primaryName:$('#deckCommander1').value,secondary:true,onSelect:d=>{$('#deckCommander2').value=d.name}});
...
async function openPreconPicker(onPick){const returnDialog=$('#setupDialog')?.open?$('#setupDialog'):($('#deckDialog')?.open?$('#deckDialog'):null);const restoreParent=()=>{closeModal();if(returnDialog&&!returnDialog.open)returnDialog.showModal()};openModal('PRECON CATALOG','<p>Loading official preconstructed deck catalog…</p>',[{label:'BACK',onClick:restoreParent},{label:'CANCEL',onClick:restoreParent}]);try{const rows=await listPrecons();$('#modalContent').innerHTML=`<div class="card-search-row"><input id="preconFilter" placeholder="Filter precons"><span>${rows.length} decks</span></div><div id="preconResults" class="card-search-results"></div>`;const renderRows=()=>{const q=$('#preconFilter').value.toLowerCase();const show=rows.filter(x=>!q||x.name.toLowerCase().includes(q));$('#preconResults').innerHTML=show.map((x,i)=>`<button class="search-result" data-precon="${i}"><span><b>${esc(x.name)}</b><br><small>${esc(x.releaseDate)} • ${esc(x.type)}</small></span></button>`).join('');$$('[data-precon]').forEach((b,i)=>b.onclick=async()=>{const row=show[i];const original=b.innerHTML;b.disabled=true;b.textContent='Loading…';try{const pre=await loadPrecon(row.fileName);await onPick(pre);closeModal();if(returnDialog&&!returnDialog.open)returnDialog.showModal()}catch(e){console.error('Commander Companion precon selection failed:',e);b.disabled=false;b.innerHTML=original;toast(e?.message||'Unable to load that preconstructed deck.',true)}})};$('#preconFilter').oninput=renderRows;renderRows()}catch(e){$('#modalContent').innerHTML=`<p class="bad">${esc(e.message)}</p>`}}
...
async function openPersistentGameLogs(){
  const returnToCommandCenter=$('#modalTitle')?.textContent==='COMMAND CENTER';
  await userDataReady;
  ...
  openModal('GAME LOGS',`${activeHtml}<section class="persistent-log-section"><h3>COMPLETED GAMES</h3>${completed||'<p class="muted">Completed-game history will appear here.</p>'}</section>`,[{label:returnToCommandCenter?'BACK':'CLOSE',semantic:returnToCommandCenter?'back':undefined,onClick:returnToCommandCenter?openLandingCommandMenu:closeModal}]);
}
