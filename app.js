...
function playerPanel(i){
  ...
  return `<section class="player-setup" data-player-setup="${i}"><div class="setup-first-line"><label>Name<input class="setup-name" value="${i===0?'Player 1':`Player ${i+1}`}"></label>${hasDeck?`<select class="setup-saved" aria-label="Load saved deck"><option value="">Load saved deck…</option>${saved.map(d=>`<option value="${esc(d.id)}">${esc(d.name)}</option>`).join('')}</select><button type="button" class="setup-view-deck">VIEW DECK</button>`:''}</div><div class="commander-line"><label>Commander 1<input class="setup-cmd1" placeholder="${commanderHint}" ${fullTracked?'readonly':''}>${fullTracked?'<button type="button" class="setup-pick-cmd1">SELECT FROM DECK</button>':selectedMode==='freeplay'?'<button type="button" class="setup-search-cmd1">SEARCH COMMANDER</button>':''}</label><label class="setup-secondary-wrap" hidden>Commander 2<input class="setup-cmd2" placeholder="Second commander" ${fullTracked?'readonly':''}>${fullTracked?'<button type="button" class="setup-pick-cmd2">SELECT LEGAL PARTNER</button>':selectedMode==='freeplay'?'<button type="button" class="setup-search-cmd2">SEARCH LEGAL PARTNER</button>':''}</label>${hasDeck?'<button type="button" class="setup-precon">PRECON</button>':''}</div>${hasDeck?'<textarea class="setup-deck" hidden aria-hidden="true"></textarea>':''}</section>`
} 
...
async function openSetup(){await userDataReady;const dialog=$('#setupDialog');closeOtherPrimaryDialogs(dialog);setupPlayerCount=2;activeSetupPlayer=0;renderSetupPanels();$('#setupProgress').textContent='';$('#virtualHand').checked=true;$('#setupDialog').showModal()}
