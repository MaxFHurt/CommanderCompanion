...
import { createTransactionEngine } from './transactions.js?v=080-al';
...
import { renderGame, renderPlayerClient, renderHostDashboard, renderTabletop, renderCardDetail, zoneModal, defOf, imageOf, playable, esc } from './ui-render.js?v=080-al';
...
import { compileEffectText, applyEffects, locateCardInGame } from './effect-engine.js?v=080-al';
...
function flexManaBadge(options=[],count=1,cls='calc-flex-pip'){
  ...
  return `<span class="${cls}" aria-label="${pair.join(' or ')} flexible mana, ${count} available"><img class="mana-flex-split" src="mana-split-${key}.png?v=080-al" alt="${pair.join(' / ')}"><b>${count}</b></span>`;
}
...
function openModal(title,html,actions=[],trayHtml=''){
  ...
  if(topBack){topBack.innerHTML='<img src="ui-back.png?v=080-al" alt="Back">';topBack.dataset.navigation='back';topBack.setAttribute('aria-label','Back');topBack.onclick=()=>backAction?.onClick?backAction.onClick(topBack):closeModal()}
