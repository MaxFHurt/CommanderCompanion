...
function openModeSetup(mode){selectedMode=mode;const title=mode==='fully-tracked'?'FULL PLAY TRACKING':mode==='freeplay'?'FREEPLAY':'TABLE TRACKER';$('#modeSetupTitle').textContent=`${title} — SETUP`;let body=`<div class="setup-stage"><h3>DEVICE SETUP</h3>`;
 ...
 $('#modeSetupContent').innerHTML=body;$('#modeSetupActions').innerHTML='<button class="dialog-close danger" data-action-role="cancel" data-menu-semantic="cancel">CANCEL</button><button id="modeProceed" class="primary">PROCEED</button>';$('#modeSetupDialog').showModal();$('#modeSetupDialog .dialog-close').onclick=()=>$('#modeSetupDialog').close();
