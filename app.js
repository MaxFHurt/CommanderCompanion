...
function openModeSetup(mode){const dialog=$('#modeSetupDialog');closeOtherPrimaryDialogs(dialog);selectedMode=mode;const title=mode==='fully-tracked'?'FULL PLAY TRACKING':mode==='freeplay'?'FREEPLAY':'TABLE TRACKER';$('#modeSetupTitle').textContent=`${title} — SETUP`;let body=`<div class="setup-stage"><h3>DEVICE SETUP</h3>`;
 ...
 $('#modeSetupContent').innerHTML=body;labelRuleOptions($('#modeSetupContent'));$('#modeSetupActions').innerHTML='<button class="dialog-close danger" data-action-role="cancel" data-menu-semantic="cancel">CANCEL</button><button id="modeProceed" class="primary">PROCEED</button>';const setupStages=$('#modeSetupContent .setup-stage');if(setupStages.length>1){setupStages[1].hidden=true;setupStages[1].classList.add('rules-stage-deferred')}$('#modeSetupDialog').showModal();$('#modeSetupDialog .dialog-close').onclick=()=>$('#modeSetupDialog').close();
