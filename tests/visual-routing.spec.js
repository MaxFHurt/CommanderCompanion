...
test.describe('V0.8 CX visual and routing tap-through',()=>{
  test('portrait is unsupported and shows only the landscape gate',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cx-landing-master.png',fullPage:false});
  ...
  test('Guided/Freeplay top rail, zones, phase and undo are routed',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cx-freeplay-iphone.png',fullPage:true});
    expect(errors).toEqual([]);
  });
  ...
  test('Table Tracker buttons and direct edits are routed',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cx-table-tracker.png',fullPage:true});
  ...
  test('iPad uses the same composition scaled to fit',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cx-freeplay-ipad.png',fullPage:true});
    expect(errors).toEqual([]);
  });
});