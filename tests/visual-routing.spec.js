...
test.describe('V0.8 CU visual and routing tap-through',()=>{
  test('portrait is unsupported and shows only the landscape gate',async({page})=>{
    ...
    await expect(page.locator('#landscapeOnlyGate')).toHaveText('Please rotate your device to landscape orientation to continue');
    await expect(page.locator('#landing')).not.toBeVisible();
    await expect(page.locator('#gameScreen')).not.toBeVisible();
    await expect(page.locator('#tabletopScreen')).not.toBeVisible();

    const gate=await page.locator('#landscapeOnlyGate').boundingBox();
    expect(gate.width).toBeLessThanOrEqual(390.5);
    expect(gate.height).toBeLessThanOrEqual(844.5);

    await page.setViewportSize({width:932,height:430});
    await page.waitForTimeout(160);
    await expect(page.locator('#landscapeOnlyGate')).not.toBeVisible();
    await expect(page.locator('#landing')).toBeVisible();
    await page.screenshot({path:'test-results/cu-landing-master.png',fullPage:false});
    expect(errors).toEqual([]);
  });

  test('landing buttons route correctly',async({page})=>{
    const errors=await ready(page);
    await expect(page).toHaveTitle(/V0\.8 CU/);
  ...
  test('Guided/Freeplay top rail, zones, phase and undo are routed',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cu-freeplay-iphone.png',fullPage:true});
    expect(errors).toEqual([]);
  });
  ...
  test('Table Tracker buttons and direct edits are routed',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cu-table-tracker.png',fullPage:true});
  ...
  test('iPad uses the same composition scaled to fit',async({page})=>{
    ...
    await page.screenshot({path:'test-results/cu-freeplay-ipad.png',fullPage:true});
    expect(errors).toEqual([]);
  });
});