...
import { applyEffects as applyGenericEffects, locateCardInGame, definitionFor, moveCard } from './effect-engine.js?v=080-bm';
import { queueTriggers } from './trigger-engine.js?v=080-az';
...
function emit(game,event){
  if(event?.type==='tapped'){const hit=locateCardInGame(game,event.sourceId);if(hit?.card){const key=`tap:${Number(game.turnNumber||0)}`;hit.card.turnEventCounts=hit.card.turnEventCounts||{};hit.card.turnEventCounts[key]=Number(hit.card.turnEventCounts[key]||0)+1;event.tapNumberThisTurn=hit.card.turnEventCounts[key];event.typeLine=event.typeLine||defFor(game,hit.card)?.typeLine||'';}}
...
function putSpellOnStack(game,player,action,deck){
  ...
  emit(game,{type:'spell-cast',sourceId:card.instanceId,definitionId:card.definitionId,controllerId:player.playerId,fromZone:from,stackId:stackObject.id,typeLine:defFor(game,card)?.typeLine||'',manaValue:Number(defFor(game,card)?.manaValue??defFor(game,card)?.cmc??0)});
  return stackObject;
}
function payAbilityCosts(game,player,action,deck){
  ...
  if(action.energyCost){player.counters=player.counters||{};if(Number(player.counters.energy||0)<Number(action.energyCost))throw new Error('Not enough Energy to pay this activation cost');player.counters.energy=Number(player.counters.energy||0)-Number(action.energyCost)}
  ...
  if(Array.isArray(action.costMoveIds))for(const id of action.costMoveIds){const mv=locate(deck,id);if(!mv)throw new Error('Chosen activation-cost card is unavailable');const def=defFor(game,mv.card),fromZone=mv.z,fromController=mv.card.controllerId||player.playerId,wasTapped=!!mv.card.tapped;const [paid]=mv.a.splice(mv.i,1);paid.zone='graveyard';paid.tapped=false;const owner=ownerFor(game,paid,player);if(!paid.token)owner.deck.graveyard.push(paid);if(fromZone==='battlefield'){emit(game,{type:'sacrificed',sourceId:paid.instanceId,definitionId:paid.definitionId,controllerId:fromController,ownerId:paid.ownerId,typeLine:def?.typeLine||'',token:!!paid.token,cost:true});emit(game,{type:'leaves-battlefield',sourceId:paid.instanceId,definitionId:paid.definitionId,controllerId:fromController,ownerId:paid.ownerId,destination:'graveyard',typeLine:def?.typeLine||'',wasTapped});if(/Creature/i.test(def?.typeLine||''))emit(game,{type:'dies',sourceId:paid.instanceId,definitionId:paid.definitionId,controllerId:fromController,ownerId:paid.ownerId,typeLine:def?.typeLine||'',token:!!paid.token})}else if(fromZone==='hand'){emit(game,{type:'discard',playerId:player.playerId,sourceId:paid.instanceId,definitionId:paid.definitionId,cost:true})}}
  if(action.sacrificeSelf){const selfHit=locate(deck,action.instanceId);if(selfHit){const def=defFor(game,selfHit.card),fromController=selfHit.card.controllerId||player.playerId,wasTapped=!!selfHit.card.tapped;const [sacrificed]=selfHit.a.splice(selfHit.i,1);sacrificed.zone='graveyard';sacrificed.tapped=false;const owner=ownerFor(game,sacrificed,player);if(!sacrificed.token)owner.deck.graveyard.push(sacrificed);emit(game,{type:'sacrificed',sourceId:sacrificed.instanceId,definitionId:sacrificed.definitionId,controllerId:fromController,ownerId:sacrificed.ownerId,typeLine:def?.typeLine||'',token:!!sacrificed.token,cost:true});emit(game,{type:'leaves-battlefield',sourceId:sacrificed.instanceId,definitionId:sacrificed.definitionId,controllerId:fromController,ownerId:sacrificed.ownerId,destination:'graveyard',typeLine:def?.typeLine||'',wasTapped});if(/Creature/i.test(def?.typeLine||''))emit(game,{type:'dies',sourceId:sacrificed.instanceId,definitionId:sacrificed.definitionId,controllerId:fromController,ownerId:sacrificed.ownerId,typeLine:def?.typeLine||'',token:!!sacrificed.token})}}
  return card;
}
...
export function createTransactionEngine(game){
  function commit(action){
    ...
    if(action.type==='cast-spell')putSpellOnStack(game,player,action,deck);
    ...
    else if(action.type==='resolve-stack'){let r;try{r=resolveStackTop(game)}catch(error){const history=game.undoHistory;restore(game,before);game.undoHistory=history;throw error}action.label=`${r.obj.label||'Stack object'} resolves.${r.notes.length?' '+r.notes.join(' '):''}`}
      ...
      emit(game,{type:'enters-battlefield',sourceId:card.instanceId,definitionId:card.definitionId,controllerId:player.playerId,ownerId:card.ownerId,typeLine:defFor(game,card)?.typeLine||'',manaValue:Number(defFor(game,card)?.manaValue??defFor(game,card)?.cmc??0)});
    }
      ...
      const hit=locate(deck,action.instanceId);if(!hit)throw new Error('Card instance not found');if(hit.z!=='battlefield')throw new Error('Only battlefield permanents can be tapped or untapped.');const card=hit.card;if(action.tapped===false&&!action.allowUntap)throw new Error('A tapped permanent can only untap during the untap step or because a rule/card effect permits it.');if(action.tapped!==false&&card.tapped)throw new Error('This permanent is already tapped.');setTappedWithCapacity(game,player,card,action.tapped!==false);emit(game,{type:card.tapped?'tapped':'untapped',sourceId:card.instanceId,definitionId:card.definitionId,controllerId:card.controllerId||player.playerId});
    }
      ...
      const hit=locate(deck,action.instanceId);if(!hit||hit.z!=='battlefield')throw new Error('Ability source must be on the battlefield');const card=hit.card;if(action.requiresTap){if(card.tapped)throw new Error('Ability source is already tapped');setTappedWithCapacity(game,player,card,true);emit(game,{type:'tapped',sourceId:card.instanceId,definitionId:card.definitionId,controllerId:player.playerId})}if(action.requiresUntap){if(!card.tapped)throw new Error('Ability source is not tapped');setTappedWithCapacity(game,player,card,false)}payMana(game,player,action.payment);if(action.lifeCost){if(player.life<action.lifeCost)throw new Error('Not enough life to pay this activation cost');player.life-=action.lifeCost}if(Array.isArray(action.costMoveIds))for(const id of action.costMoveIds){const mv=locate(deck,id);if(!mv)throw new Error('Chosen activation-cost card is unavailable');const def=defFor(game,mv.card),fromZone=mv.z,fromController=mv.card.controllerId||player.playerId,wasTapped=!!mv.card.tapped;const [paid]=mv.a.splice(mv.i,1);paid.zone='graveyard';paid.tapped=false;const owner=ownerFor(game,paid,player);if(!paid.token)owner.deck.graveyard.push(paid);if(fromZone==='battlefield'){emit(game,{type:'sacrificed',sourceId:paid.instanceId,definitionId:paid.definitionId,controllerId:fromController,ownerId:paid.ownerId,typeLine:def?.typeLine||'',token:!!paid.token,cost:true});emit(game,{type:'leaves-battlefield',sourceId:paid.instanceId,definitionId:paid.definitionId,controllerId:fromController,ownerId:paid.ownerId,destination:'graveyard',typeLine:def?.typeLine||'',wasTapped});if(/Creature/i.test(def?.typeLine||''))emit(game,{type:'dies',sourceId:paid.instanceId,definitionId:paid.definitionId,controllerId:fromController,ownerId:paid.ownerId,typeLine:def?.typeLine||'',token:!!paid.token})}else if(fromZone==='hand'){emit(game,{type:'discard',playerId:player.playerId,sourceId:paid.instanceId,definitionId:paid.definitionId,cost:true})}}if(action.sacrificeSelf){const selfHit=locate(deck,action.instanceId);if(selfHit&&selfHit.z==='battlefield')moveCard(game,action.instanceId,'graveyard',{moveReason:'sacrifice',onEvent:e=>emit(game,{...e,cost:true})})}applySearchResult(game,player,action);const effectNotes=applyTrackedEffects(game,player,action.effects||[],{...action.effectBindings,sourceId:action.instanceId});if(effectNotes.length)action.label=`${action.label||'Ability resolved.'} ${effectNotes.join(' ')}`;
    }
    ...
    else if(action.type==='life'){const before=player.life;player.life=Math.max(0,player.life+action.delta);const delta=player.life-before;player.lifeFeedback=delta?{direction:delta>0?'gain':'loss',until:Date.now()+20000}:null;emit(game,{type:delta>=0?'life-gained':'life-lost',playerId:player.playerId,amount:Math.abs(delta),controllerId:player.playerId})}
      ...
      resetManaAtBoundary(game);if(action.phase==='postcombat-main')cleanupEndCombatEffects(game);game.phase=action.phase;configurePhaseGates(game,action.phase);if(action.phase==='upkeep')emit(game,{type:'upkeep',playerId:game.activePlayerId});if(action.phase==='precombat-main')emit(game,{type:'precombat-main-start',playerId:game.activePlayerId,controllerId:game.activePlayerId});if(action.phase==='end-step')emit(game,{type:'end-step',playerId:game.activePlayerId});if(action.phase==='begin-combat'||action.phase==='combat')emit(game,{type:'begin-combat',playerId:game.activePlayerId});
    }
