import { validatePlay, validateAttack, availableActivatedAbilities, validateActivatedAbilityFull, playerManaAvailability } from './rules-v0725.js?v=080-ai';
...
function manaBox(game,title,p,pool,interactive=true){const availability=playerManaAvailability(p,game),available=p.mana?.available||{},total=p.mana?.total||{},allowed=legalManaKeys(game,p);for(const [k] of MANA)if(Number(available[k]||0)>0||Number(total[k]||0)>0)allowed.add(k);for(const src of availability.__flex||[])for(const k of src.options||[])allowed.add(k);const colors=MANA.filter(([k])=>allowed.has(k));const groups=new Map();for(const src of availability.__flex||[]){const opts=(src.options||[]).filter(k=>MANA.some(([mk])=>mk===k));if(opts.length>1){const key=opts.join('/');groups.set(key,(groups.get(key)||0)+1)}}const flex=[...groups.entries()].map(([key,count])=>{const opts=key.split('/'),icons=opts.slice(0,2).map(k=>{const n=MANA.find(x=>x[0]===k)?.[1]||'colorless';return `<img src="mana-${n}.png" alt="${k}">`}).join('');return `<span class="mana-pip mana-flex" aria-label="${key} flexible mana source, ${count} available"><span class="mana-flex-icons">${icons}</span><b>${count}</b></span>`}).join('');return `<button class="mana-box mana-box-button available-only" ${interactive?'data-mana-pool="available"':'aria-disabled="true"'} aria-label="Available mana"><div class="mana-row" style="--mana-count:${Math.max(1,colors.length+groups.size)}">${colors.map(([k,n])=>{const a=Math.max(0,Number(available[k]||0)),t=Math.max(0,Number(total[k]||0)),state=a>t?'mana-over':a<t?'mana-under':'mana-even';return `<span class="mana-pip ${state}"><img src="mana-${n}.png" alt="${k}"><b>${a}</b></span>`}).join('')}${flex}</div></button>`}
