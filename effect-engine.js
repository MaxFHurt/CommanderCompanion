...
export function moveCard(game,id,to,{controllerId=null,position='bottom',tapped=false,preserveCounters=false,onEvent=null,moveReason=null}={}){
  ...
  if(moveReason==='sacrifice'&&from==='battlefield')onEvent?.({type:'sacrificed',sourceId:card.instanceId,definitionId:card.definitionId,controllerId:fromController,ownerId:card.ownerId,typeLine:definition?.typeLine||'',token:!!card.token});
...
function applyEffectsUnsafe(game,controllerId,effects=[],bindings={}){
  ...
  for(const effect of effects||[]){
    ...
    if(effect.kind==='damage-from-creature'||effect.kind==='damage-from-selected-creature'){
      ...
      if(effect.sacrificeSource){const live=locateCardInGame(game,sourceId);if(live?.zone==='battlefield'){moveCard(game,sourceId,'graveyard',{moveReason:'sacrifice',onEvent:e=>emit(bindings,e)});notes.push(`${sd?.name||'Creature'} is sacrificed.`)}}continue;
    }
    ...
    if(effect.kind==='sacrifice-many'){
      ...
      for(const p of players){const ids=Array.isArray(raw)?raw:(raw[p.playerId]||[]);if(ids.length<amount)throw new Error(`${p.displayName} must choose ${amount} permanent${amount===1?'':'s'} to sacrifice.`);for(const id of ids.slice(0,amount)){const hit=locateCardInGame(game,id);if(!hit||hit.zone!=='battlefield'||(hit.card.controllerId||hit.player.playerId)!==p.playerId)throw new Error('A sacrifice choice is no longer legal.');const n=cardName(game,hit.card);moveCard(game,id,'graveyard',{moveReason:'sacrifice',onEvent:e=>emit(bindings,e)});notes.push(`${p.displayName} sacrifices ${n}.`)}}continue;
    }
    if(effect.kind==='counter'){
      const targetId=resolveBoundId(effect,bindings);const hit=locateCardInGame(game,targetId);if(!hit||hit.zone!=='battlefield')throw new Error('The counter target is no longer legal.');const oldValue=Number(hit.card?.counters?.[effect.counter||'+1/+1']||0);addCounter(hit.card,effect.counter||'+1/+1',amount);const newValue=Number(hit.card?.counters?.[effect.counter||'+1/+1']||0);notes.push(`${cardName(game,hit.card)} gets ${amount} ${effect.counter||'+1/+1'} counter${amount===1?'':'s'}.`);emit(bindings,{type:'counter-added',targetId,controllerId:hit.card.controllerId,sourceId:bindings.sourceId||null,counter:effect.counter||'+1/+1',amount,oldValue,newValue});checkLethal(game,targetId,notes,bindings);continue;
    }
    ...
    if(effect.kind==='add-mana'){
      const color=effect.color||bindings[effect.colorBind||'manaColor'];if(!['W','U','B','R','G','C'].includes(color))throw new Error('A legal mana color/type must be chosen.');controller.mana.available[color]=Number(controller.mana.available[color]||0)+amount;controller.mana.floating=controller.mana.floating||{W:0,U:0,B:0,R:0,G:0,C:0};controller.mana.floating[color]=Number(controller.mana.floating[color]||0)+amount;notes.push(`${controller.displayName} adds ${amount} ${color} mana.`);continue;
    }
